...
todays_date_formatted <- as.Date("2019-09-04")
...
data %>% filter(!is.na(s_mfq1w_tot) | !is.na(p_f_mfq1w_tot) | !is.na(p_m_mfq1w_tot) | !is.na(p_u_mfq1w_tot)) %>% filter(Clinical_Visit_Number != '0') %>% 
  ...
  ggplot(aes(x=Clinical_Visit_Number, y=value, group=Measure), na.rm = TRUE) +
    # ggplot(aes(x=Clinical_Visit_Number, y=value, colour=Measure), na.rm = TRUE) +
  # geom_line() +
  ...
  ylim(0,27) + 
  # scale_color_manual(labels = c("Father", "Mother", "Parent - unknown", "Child"), values = c("#666666", "#666666", "#999999", "#000000")) +
  scale_shape_manual(labels = c("Father", "Mother", "Parent: not recorded", "Child"), values=c(17, 16, 3, 5)) +
  theme_minimal() +
...
data %>% filter(!is.na(s_mfq1w_tot) | !is.na(p_f_mfq1w_tot) | !is.na(p_m_mfq1w_tot) | !is.na(p_u_mfq1w_tot)) %>%  filter(Clinical_Visit_Number != '0') %>% 
  select(Clinical_Visit_Date, Clinical_Visit_Number, s_mfq1w_tot, p_f_mfq1w_tot, p_m_mfq1w_tot, p_u_mfq1w_tot) %>% mutate(s_mfq1w_tot = replace_na(s_mfq1w_tot, " ")) %>% 
  mutate(p_f_mfq1w_tot = replace_na(p_f_mfq1w_tot, " ")) %>% mutate(p_m_mfq1w_tot = replace_na(p_m_mfq1w_tot, " ")) %>% mutate(p_u_mfq1w_tot = replace_na(p_u_mfq1w_tot, " ")) %>% 
...
data %>% filter(!is.na(s_scared_tot) | !is.na(p_f_scared_tot) | !is.na(p_m_scared_tot) | !is.na(p_u_scared_tot)) %>%  filter(Clinical_Visit_Number != '0') %>% 
  select(Clinical_Visit_Date, Clinical_Visit_Number, s_scared_tot, p_f_scared_tot, p_m_scared_tot, p_u_scared_tot) %>% mutate(s_scared_tot = replace_na(s_scared_tot, " ")) %>% 
  mutate(p_f_scared_tot = replace_na(p_f_scared_tot, " ")) %>% mutate(p_m_scared_tot = replace_na(p_m_scared_tot, " ")) %>% mutate(p_u_scared_tot = replace_na(p_u_scared_tot, " ")) %>% 
...
data %>% filter(!is.na(s_ari1w_tot) | !is.na(p_f_ari1w_tot) | !is.na(p_m_ari1w_tot) | !is.na(p_u_ari1w_tot)) %>% filter(Clinical_Visit_Number != '0') %>% 
  select(Clinical_Visit_Date, Clinical_Visit_Number, s_ari1w_tot, p_f_ari1w_tot, p_m_ari1w_tot, p_u_ari1w_tot) %>% mutate(s_ari1w_tot = replace_na(s_ari1w_tot, " ")) %>% 
  mutate(p_f_ari1w_tot = replace_na(p_f_ari1w_tot, " ")) %>% mutate(p_m_ari1w_tot = replace_na(p_m_ari1w_tot, " ")) %>% mutate(p_u_ari1w_tot = replace_na(p_u_ari1w_tot, " ")) %>% 
