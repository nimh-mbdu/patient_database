...
Over the course of his treatment, Leo and his parents, `r p_f_name` `r p_l_name` and `r p_f_name2` `r p_l_name2`, ...*INSERT*

At the end of treatment, `r f_name` and `pronoun` parents reported that he had more ability to ...*INSERT*
...
data %>% filter(!is.na(s_mfq1w_tot) | !is.na(p_mfq1w_tot)) %>% filter(Clinical_Visit_Number != '0') %>% 
  ...
  mutate(p_mfq1w_parent = replace_na(p_mfq1w_parent, " ")) %>% mutate(Clinical_Visit_Number = as.character(Clinical_Visit_Number)) %>%
...
data %>% filter(!is.na(s_scared_tot) | !is.na(p_scared_tot)) %>%  filter(Clinical_Visit_Number != '0') %>%
  ...
  mutate(p_scared_parent = replace_na(p_scared_parent, " ")) %>% mutate(Clinical_Visit_Number = as.character(Clinical_Visit_Number)) %>%
  kable(., col.names = c("Visit Date", "Visit Number", "Child reporting on self", "Parent reporting on child", "Parent"))
...
data %>% filter(!is.na(s_shaps_tot)) %>%  filter(Clinical_Visit_Number != '0') %>%  arrange(Clinical_Visit_Date) %>% 
  ...
  mutate(s_shaps_tot = replace_na(s_shaps_tot, " ")) %>% mutate(Clinical_Visit_Number = as.character(Clinical_Visit_Number)) %>% 
...
data %>% filter(!is.na(s_ari1w_tot) | !is.na(p_ari1w_tot)) %>% filter(Clinical_Visit_Number != '0') %>%
  ...
  mutate(p_ari1w_parent = replace_na(p_ari1w_parent, " ")) %>% mutate(Clinical_Visit_Number = as.character(Clinical_Visit_Number)) %>%
  kable(., col.names = c("Visit Date", "Visit Number", "Child reporting on self", "Parent reporting on child", "Parent"))
...
data %>% filter(!is.na(s_lsasad_tot)) %>% filter(Clinical_Visit_Number != '0') %>% arrange(Clinical_Visit_Date) %>% 
  ...
  mutate(s_lsasad_tot = replace_na(s_lsasad_tot, " ")) %>% mutate(Clinical_Visit_Number = as.character(Clinical_Visit_Number)) %>% 
  kable(., col.names = c("Visit Date", "Visit Number", "Child reporting on self"))
...
data %>% filter(!is.na(c_cdrs_tot)) %>% arrange(Clinical_Visit_Date) %>% select(Clinical_Visit_Date4, Clinical_Visit_Number, c_cdrs_tot) %>% 
  mutate(c_cdrs_tot = replace_na(c_cdrs_tot, " ")) %>% mutate(Clinical_Visit_Number = as.character(Clinical_Visit_Number)) %>% 
