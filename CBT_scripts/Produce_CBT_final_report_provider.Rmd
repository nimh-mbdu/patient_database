...
# # directories -------------------------------------------------------------
# 
# todays_date_formatted <- as.Date("2019-08-28")
...
data %>% filter(!is.na(c_cdrs_tot)) %>% gather(Measure, value, c_cdrs_tot) %>% 
  ...
  geom_point(size=3) +
  geom_text(aes(label=value), hjust=0, vjust=-1) +
  labs(title="Depression symptoms\n", x="\nDate of visit (MM-DD-YYYY)", y="CDRS Total\n") + 
  scale_x_date(date_labels = "%m-%d-%Y") +
  ylim(0,125) + 
  scale_color_manual(labels = c("Consensus"), values = c("#339900")) +
  theme_minimal() +
  guides(color=guide_legend("Person reporting:")) 
...
data %>% filter(!is.na(c_cdrs_tot)) %>% select(Clinical_Visit_Date, Clinical_Visit_Number, c_cdrs_tot) %>% 
