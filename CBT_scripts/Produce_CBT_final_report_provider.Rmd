...
numeric_variables <- CBT_report %>% select(matches("_tot"), Clinical_Visit_Number) %>% colnames() 
...
data %>% filter(!is.na(s_lsas_tot) | !is.na(s_lsasad_tot)) %>% filter(Clinical_Visit_Number != '0') %>% 
  gather(Measure, value, s_lsas_tot, s_lsasad_tot) %>%
  ggplot(aes(x=Clinical_Visit_Number, y=value, group=Measure), na.rm = TRUE) +
  geom_point(aes(shape=Measure, group=Measure), size=3) +
  geom_line() +
  geom_text(aes(label=value), hjust=0, vjust=-1) +
  ...
  ylim(0,150) + 
  scale_shape_manual(labels = c("Adult", "Adolescent"), values=c(17, 16)) +
  scale_x_continuous(breaks=seq(0, nrow(data), 1)) +
  theme_minimal() +
  guides(shape=guide_legend("LSAS Type:"))
...
data$s_lsas_tot <- coalesce(data$s_lsas_tot, data$s_lsasad_tot)
data %>% filter(!is.na(s_lsas_tot)) %>%  filter(Clinical_Visit_Number != '0') %>% arrange(Clinical_Visit_Date) %>% 
  select(Clinical_Visit_Date4, Clinical_Visit_Number, s_lsas_tot) %>%
  kable(., col.names = c("Visit Date", "Visit Number", "Child reporting on self"))
