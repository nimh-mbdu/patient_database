...
referrals_final %>%
  ...
  geom_text(aes(x=as.Date("2019-02-08"), label="189,000\n", y=22), colour="#000000", angle=90, text=element_text(size=9)) +
  geom_text(aes(x=as.Date("2019-04-26"), label="108,000\n", y=22), colour="#FF0000", angle=90, text=element_text(size=9)) +
...
baseline <- Psychometrics_treatment %>% 
  select(Initials, IRTA_tracker, Participant_Type2, Protocol, Eligible, Clinical_Visit_Date, Clinical_Visit_Type, c_ksadsdx_visit_type, c_ksadsdx_date) %>% 
...
baseline_removed <- baseline %>% group_by(Initials) %>% arrange(Clinical_Visit_Date) %>% filter(IRTA_tracker=="REMOVED" | n()==1) %>% ungroup() %>% 
  filter(IRTA_tracker=="REMOVED" | !str_detect(Protocol, "0037") | str_detect(Protocol, "SCREENING") |
           Eligible =="4" | Eligible =="5" | Eligible =="6" | Eligible =="7") 
removed_initials <- baseline_removed$Initials
...
overdue <- baseline_keep %>% filter(status==1) %>% select(Initials, IRTA_tracker, Participant_Type2, expt_FU_date_TDiff)
...
overdue %>% arrange(expt_FU_date_TDiff) %>% kable(., col.names = c("Initials", "IRTA", "Diagnosis", "Days overdue")) %>%
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
