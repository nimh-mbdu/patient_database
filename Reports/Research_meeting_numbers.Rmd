...
if (exists("Psychometrics_treatment")==FALSE) {
        ...
        covid19 <- Psychometrics_treatment %>% select(Initials, IRTA_tracker, s_covid19_date, s_covid19_tot, s_covid19_12a_infected_self, s_covid19_12b_family_infected) %>% 
          filter(!is.na(s_covid19_tot))
...
##### COVID19 questionnaire completion:

Numbers as of last SDQ+ pull (`r Psychometrics_date`). 

```{r echo=FALSE, warning=FALSE, message=FALSE, results="hide" }

covid19_latest <- covid19 %>% group_by(Initials) %>% arrange(s_covid19_date) %>% slice(n()) %>% ungroup() %>% filter(s_covid19_date > last_week_date_formatted)
covid19_latest[,5:6] <- lapply(covid19_latest[,5:6], FUN = function(x) recode(x, `1`= "Yes", `0`= "No", .missing = NULL))
covid19_latest[,5:6] <- sapply(covid19_latest[,5:6], replace_na, " ")
  
covid19_table <- covid19 %>% group_by(Initials, IRTA_tracker) %>% tally()
covid19_table_IRTA <- covid19 %>% group_by(Initials) %>% arrange(s_covid19_date) %>% slice(n()) %>% ungroup() %>% group_by(IRTA_tracker) %>% tally()
...
covid19_table_IRTA %>% kable(., col.names = c("IRTA", "Freq")) %>%
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
...
covid19_table %>% kable(., col.names = c("Initials", "IRTA", "Freq")) %>%
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))

```

Latest scores (where questionnaire max total = 230 from 03-23-2020; measures collected prior to this had a max score of 150):

```{r echo=FALSE, warning=FALSE, message=FALSE}

covid19_latest %>% kable(., col.names = c("Initials", "IRTA", "Date completed", "Total", "Infected (self)", "Infected (family)")) %>% 
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
...
rm(screens_table2, screens_current_table, ready_2_review, scheduled, scheduling_table2, protocol_table2, 
   ...
   current_outpatients, current_inpatients, outpatients, outpatients_table, inpatients, family_hist_table2, covid19_table, 
