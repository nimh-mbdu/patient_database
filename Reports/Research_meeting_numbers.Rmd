...
if (exists("Psychometrics_treatment")==FALSE) {
        ...
        crisis <- Psychometrics_treatment %>% select(Initials, IRTA_tracker, Clinical_Visit_Date,
                 ...
                 s_mfq1w_tot, p_mfq1w_tot, s_crisis_base_1_exposed, s_crisis_base_2_self_diagnosis, 
                 s_crisis_fu_1_exposed, s_crisis_fu_2_self_diagnosis) %>% 
          filter(!is.na(s_crisis_3m_tot) | !is.na(s_crisis_base_tot) |!is.na(s_crisis_fu_tot) |!is.na(s_mfq1w_tot))
...
baseline <- latest_eligibility %>% select(Initials, Eligible) %>% merge.default(baseline, ., all=TRUE)
...
rm(baseline, baseline_removed, removed_initials, FU_completed, baseline_keep, temp1, temp2, rest_temp, date_temp, descrip_table, 
   MDD_one_year, MDD_two_year, HV_one_year, HV_two_year)
...
s_crisis_baseline <- crisis %>% select(Initials, IRTA_tracker, Clinical_Visit_Date, s_crisis_base_tot, s_mfq1w_tot, 
...
s_crisis_followup <- crisis %>% select(Initials, IRTA_tracker, Clinical_Visit_Date, s_crisis_fu_tot, s_mfq1w_tot, 
...
p_crisis_baseline <- crisis %>% select(Initials, IRTA_tracker, Clinical_Visit_Date, p_crisis_base_tot, p_mfq1w_tot) %>% 
  rename(p_crisis_tot = "p_crisis_base_tot") %>% mutate(type = "last two weeks (baseline)") %>% filter(!is.na(p_crisis_tot))
p_crisis_followup <- crisis %>% select(Initials, IRTA_tracker, Clinical_Visit_Date, p_crisis_fu_tot, p_mfq1w_tot) %>% 
...
rm(screens_table2, screens_current_table, ready_2_review, scheduled, scheduling_table2, protocol_table2, 
