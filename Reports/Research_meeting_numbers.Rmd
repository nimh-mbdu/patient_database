...
if (exists("Psychometrics_treatment")==FALSE) {
        ...
        covid19 <- Psychometrics_treatment %>% select(Initials, IRTA_tracker, s_covid19_date, s_covid19_tot, s_covid19_11_exposed, s_covid19_12_infected) %>% filter(!is.na(s_covid19_tot))
...
Latest scores (where questionnaire max total = 150):

```{r echo=FALSE, warning=FALSE, message=FALSE}

covid19_latest %>% kable(., col.names = c("Initials", "IRTA", "Date completed", "Total", "Exposed", "Infected")) %>% 
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
