...
last_CR <- as.Date("2019-07-31")
cut_off <- as.Date("2020-07-31")
...
protocol_0037_latest <- master_IRTA_latest %>% filter(str_detect(Protocol, "0037")) %>% group_by(Initials) %>% 
  slice(n()) %>% ungroup() %>% select(Initials, FIRST_NAME, LAST_NAME, PLUSID, IRTA_tracker, Participant_Type2, SEX, 
    Eligible, Eligibility_notes, Clinical_Visit_Type, Clinical_Visit_Date, Consent_Date, Protocol, 
    Referral_Date, Screening_Start_Date, Parent_e_consented, Child_e_assented, Parent_DAWBA_completed, Child_DAWBA_completed) %>% 
  mutate(status = NA) %>% mutate(In_0037_screening=4)
...
#### Completed face-to-face screening:
...
eval_visit_overall <- protocol_0037_first %>% filter(status=="Screening") %>% filter(Overall_date <= cut_off) %>% 
  group_by(Eligible) %>% summarise(Eligible_n=n()) 
...
protocol_0037_table <- rbind(protocol_0037_table, total) %>%
  cbind(., order=c(1, 2, 11, 3, 4, 5, 6, 7, 8, 9, 10, 12)) %>% arrange(order) %>% select(-order) %>% mutate_all(as.character)
rm(total, sum)
...
##### But did not sign into characterization on the day:

I.e. signed 0037 in-person screening consents only. 
