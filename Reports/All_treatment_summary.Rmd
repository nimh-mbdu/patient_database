...
current_outpatients <- CBT_report %>% group_by(Initials) %>% arrange(Clinical_Visit_Date) %>% slice(n()) %>% ungroup() %>% 
  filter(Eligible=="Include" | is.na(Eligible)) %>% filter(!str_detect(Clinical_Visit_Type, "o0")) %>% 
...
psychometric_data_i <- MATCH_tracker %>% filter(Initials %in% inpatient_list) %>% 
  select(Initials, Clinical_Visit_Date, Clinical_Visit_Date2, Clinical_Visit_Date3, Clinical_Visit_Type, Clinical_Visit_Number, 
         s_mfq1w_tot, s_ari1w_tot, s_scared_tot, s_shaps_tot, s_lsas_tot, c_cybocs_ob_tot, c_cybocs_com_tot, c_ygtss_severity_tot, c_ygtss_tot, 
...
psychometric_data_o <- CBT_report %>% filter(Initials %in% outpatient_list) %>% 
  select(Initials, Clinical_Visit_Date, Clinical_Visit_Date2, Clinical_Visit_Date3, Clinical_Visit_Type, Clinical_Visit_Number, 
         s_mfq1w_tot, s_ari1w_tot, s_scared_tot, s_shaps_tot, s_lsas_tot)
...
for(i in seq(nrow(current_inpatients))) {
  ...
  questionnaire_data <- flextable(temp_psych, col_keys = c("Clinical_Visit_Number", "Clinical_Visit_Date3", "s_mfq1w_tot", 
      "s_ari1w_tot", "s_scared_tot", "s_shaps_tot", "s_lsas_tot", "c_cybocs_ob_tot", "c_cybocs_com_tot", "c_ygtss_severity_tot", "c_ygtss_tot", 
      "s_chocir_symptom_tot", "s_chocir_impairment_tot")) %>% 
    set_header_labels(., Clinical_Visit_Number="Week",  Clinical_Visit_Date3="Date", s_mfq1w_tot="MFQ", s_ari1w_tot="ARI",
       s_scared_tot="SCARED", s_shaps_tot="SHAPS", s_lsas_tot="LSAS", c_cybocs_ob_tot="CYBOCS:O", c_cybocs_com_tot="CYBOCS:C", 
    ...
    colformat_num(j = c("Clinical_Visit_Number", "Clinical_Visit_Date3", "s_mfq1w_tot", "s_ari1w_tot", "s_scared_tot", "s_shaps_tot", 
       "s_lsas_tot", "c_cybocs_ob_tot", "c_cybocs_com_tot", "c_ygtss_severity_tot", "c_ygtss_tot", "s_chocir_symptom_tot", "s_chocir_impairment_tot"), digits = 0, na_str = "") %>% 
...
for(i in seq(nrow(current_outpatients))) {
  ...
  cat("  \nQuestionnaire totals:")
  questionnaire_data <- flextable(temp_psych, col_keys = c("Clinical_Visit_Number", "Clinical_Visit_Date3", "s_mfq1w_tot", 
      "s_ari1w_tot", "s_scared_tot", "s_shaps_tot", "s_lsas_tot")) %>% 
    set_header_labels(., Clinical_Visit_Number="Week",  Clinical_Visit_Date3="Date", s_mfq1w_tot="MFQ", s_ari1w_tot="ARI",
       s_scared_tot="SCARED", s_shaps_tot="SHAPS", s_lsas_tot="LSAS") %>% 
    colformat_num(j = c("Clinical_Visit_Number", "Clinical_Visit_Date3", "s_mfq1w_tot", "s_ari1w_tot", "s_scared_tot",
      "s_shaps_tot", "s_lsas_tot"), digits = 0, na_str = "") %>% 
    fontsize(., size=10, part="all") %>% align(., align = "left", part="all") %>%
    FitFlextableToPage3(.)
  cat(knit_print(questionnaire_data))
