...
scaredshort_subset <- scaredshort_subset %>% filter(!is.na(c_ksadsdx_primary_dx)) %>% filter(id!=4 & id!=5)

scaredshort_group_means <- scaredshort_subset %>% group_by(c_ksadsdx_primary_dx, id, row_label) %>% 
  summarise(s_scaredshort_mean=mean(s_scaredshort_tot), s_scaredshort_sd=sd(s_scaredshort_tot), s_scaredshort_N=n()) %>%
  mutate(s_scaredshort_mean=round(s_scaredshort_mean, digit=2), s_scaredshort_sd=round(s_scaredshort_sd, digit=2)) %>% ungroup()
...
scaredshort_group_means$s_scaredshort_combined <- paste(scaredshort_group_means$s_scaredshort_mean, " (", scaredshort_group_means$s_scaredshort_sd, ")", sep="")
scaredshort_group_means_t <- scaredshort_group_means %>% 
  pivot_wider(., id_cols = c("id", "row_label"), names_from = "c_ksadsdx_primary_dx", values_from = c("s_scaredshort_combined", "s_scaredshort_N")) %>% 
  select(id, row_label, s_scaredshort_combined_Healthy, s_scaredshort_N_Healthy, s_scaredshort_combined_MDD, 
         s_scaredshort_N_MDD, `s_scaredshort_combined_Sub-MDD`, `s_scaredshort_N_Sub-MDD`)

scaredshort_group_means_t %>% kable(., col.names = c("Time point", "Type", "Healthy: M (SD)", "N", "MDD: M (SD)", "N", "Sub-MDD: M (SD)", "N")) %>% 
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
...
ggplot() +
  geom_line(data=scaredshort_group_means, aes(x=row_label, y=s_scaredshort_mean, group=c_ksadsdx_primary_dx, color=c_ksadsdx_primary_dx), size=2) + 
  geom_line(data = scaredshort_subset, aes(x=row_label, y=s_scaredshort_tot, group=Initials, color=c_ksadsdx_primary_dx), alpha=0.25) +
  labs(x="\nTime point", y="SCARED (short) total\n") +
  # labs(title="Depression symptoms\n", x="\nTime point", y="SCARED (short) total\n") +
  scale_color_manual(values=c("#CC0000", "#006600", "#0000CC")) + 
  scale_x_discrete(expand = c(0,0), limit = c("baseline", "followup")) + 
  theme_minimal() +
  guides(color=guide_legend("Diagnosis:"))
