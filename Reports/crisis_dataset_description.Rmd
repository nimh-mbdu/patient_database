...
crisis_final$row_label <- recode(crisis_final$id, `1`="baseline", `2`="pre-covid", 
                          `3`="crisis_t1", `4`="crisis_t2", `5`="crisis_t3", `6`="crisis_t4")
...
crisis_subset <- merge.default(crisis_subset1, crisis_subset2, all = TRUE) %>% merge.default(., crisis_subset3, all = TRUE) %>% 
  merge.default(., crisis_subset4, all = TRUE) %>% merge.default(., crisis_subset5, all = TRUE) 
...
ggplot() +
  ...
  scale_x_discrete(expand = c(0,0), limit = c("pre-covid", "baseline", "followup1", "followup2", "followup3")) + 
  theme_minimal() +
...
rm(parent_contacted, child_contacted, parent_agreed, child_agreed, all_table, crisis_table, measures_combined_long, 
