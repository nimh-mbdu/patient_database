...
parent_completed_all_both5 <- merge.default(parent_completed_all_both2, parent_completed_all_fu5, all=TRUE) %>%
...
child_completed_all_both5 <- merge.default(child_completed_all_both2, child_completed_all_fu5, all=TRUE) %>%
...
parent_completed_crisis_both5 <- merge.default(parent_completed_crisis_both2, parent_completed_crisis_fu5, all=TRUE) %>%
...
child_completed_crisis_both5 <- merge.default(child_completed_crisis_both2, child_completed_crisis_fu5, all=TRUE) %>%
...
s_measures_completed <- Psychometrics_treatment %>% 
  select(Initials, SDAN, PLUSID, Participant_Type2, SEX, Age_at_visit, IRTA_tracker, Clinical_Visit_Date, Clinical_Visit_Type, c_ksadsdx_primary_dx, c_ksadsdx_dx_detailed, 
    s_mfq1w_tot, s_mfq1w_date, s_mfq_tot, s_mfq_date, s_scared_tot, s_scared_date, s_scaredshort_tot, s_scaredshort_date, matches("s_crisis_"), s_covid19_date) %>%
...
p_measures_completed <- Psychometrics_treatment %>% 
  select(Initials, SDAN, PLUSID, Participant_Type2, SEX, Age_at_visit, IRTA_tracker, Clinical_Visit_Date, Clinical_Visit_Type, c_ksadsdx_primary_dx, c_ksadsdx_dx_detailed, 
    p_mfq1w_tot, p_mfq1w_date, p_mfq_tot, p_mfq_date, p_scared_tot, p_scared_date, p_scaredshort_tot, p_scaredshort_date, matches("p_crisis_"), s_covid19_date) %>%
...
measures_completed_combined <- measures_completed %>% 
  select(Initials, SDAN, PLUSID, row_label, Participant_Type2, SEX, Age_at_visit, IRTA_tracker, Clinical_Visit_Date, Clinical_Visit_Type,
    c_ksadsdx_primary_dx, c_ksadsdx_dx_detailed, s_mfq_tot, s_mfq_date, s_scared_tot, s_scared_date, s_scaredshort_tot, s_scaredshort_date, 
    p_mfq_tot, p_mfq_date, p_scared_tot, p_scared_date, p_scaredshort_tot, p_scaredshort_date, matches("s_crisis_"), matches("p_crisis_")) %>% 
...
sscared_tminus1_long <- Psychometrics_treatment %>% select(Initials, SDAN, PLUSID, Participant_Type2, SEX, Age_at_visit,  IRTA_tracker, Clinical_Visit_Date, Clinical_Visit_Type,
  c_ksadsdx_primary_dx, c_ksadsdx_dx_detailed, s_scared_tot, s_scared_date) %>% 
...
s_measures_combined_long <- merge.default(smfq_tminus1_long, sscared_tminus1_long, all=TRUE) %>% group_by(Initials) %>% arrange(Initials, desc(Clinical_Visit_Date)) %>% 
  fill(., s_mfq_tot:s_scared_date, .direction = "down") %>% fill(., s_mfq_tot:s_scared_date, .direction = "up") %>% slice(1) %>% ungroup() %>% 
...
p_measures_combined_long <- merge.default(pmfq_tminus1_long, pscared_tminus1_long, all=TRUE) %>% group_by(Initials) %>% arrange(Initials, desc(Clinical_Visit_Date)) %>% 
  fill(., p_mfq_tot:p_scared_date, .direction = "down") %>% fill(., p_mfq_tot:p_scared_date, .direction = "up") %>% slice(1) %>% ungroup() %>% 
  filter(Initials %in% measures_completed$Initials) %>% mutate(row_label = "pre-covid")
...
measures_combined_long <- merge.default(s_measures_combined_long, p_measures_combined_long, all = TRUE) %>% 
  ...
  fill(., s_mfq_tot:p_scared_date, .direction = "down") %>% fill(., s_mfq_tot:p_scared_date, .direction = "up") %>% slice(1) %>% ungroup() 
...
# SCARED short scaling ----------------------------------------------------

# scaling the SCARED short questionnaire & combining it with the original SCARED to have it all in one column
CRISIS_data_presentation_final <- CRISIS_data_presentation_final %>%
  mutate(s_scaredshort_tot_scaled=((s_scaredshort_tot/9)*41)) %>%
  mutate(s_scaredshort_tot_scaled=round(s_scaredshort_tot_scaled, digits = 0)) %>%
  mutate(p_scaredshort_tot_scaled=((p_scaredshort_tot/9)*41)) %>%
  mutate(p_scaredshort_tot_scaled=round(p_scaredshort_tot_scaled, digits = 0))
CRISIS_data_presentation_final$s_scared_combined_tot <- coalesce(CRISIS_data_presentation_final$s_scared_tot, CRISIS_data_presentation_final$s_scaredshort_tot_scaled)
CRISIS_data_presentation_final$s_scared_combined_date <- coalesce(CRISIS_data_presentation_final$s_scared_date, CRISIS_data_presentation_final$s_scaredshort_date)
CRISIS_data_presentation_final$p_scared_combined_tot <- coalesce(CRISIS_data_presentation_final$p_scared_tot, CRISIS_data_presentation_final$p_scaredshort_tot_scaled)
CRISIS_data_presentation_final$p_scared_combined_date <- coalesce(CRISIS_data_presentation_final$p_scared_date, CRISIS_data_presentation_final$p_scaredshort_date)
