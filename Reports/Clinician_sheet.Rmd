...
# todays_date_formatted <- as.Date("2019-11-27")
...
common_identifiers_screening <- master_IRTA_screens_latest %>% 
  select(FIRST_NAME, LAST_NAME, Initials, Participant_Type, IRTA_tracker, PLUSID, DAWBA_ID, DOB, Age_at_visit, SEX, 
         Eligible, Scheduling_status, Clinical_Visit_Date, Clinical_Visit_Type, Referral_Date, Screening_Start_Date, 
...
sdq_plus_tracker <- sdq_plus_tracker %>% 
  select(FIRST_NAME, LAST_NAME, Initials, Participant_Type, IRTA_tracker, PLUSID, DAWBA_ID, DOB, Age_at_visit, SEX, City, State, Marital_Status,
         Eligible, Scheduling_status, Clinical_Visit_Date, Clinical_Visit_Type, Referral_Date, Screening_Start_Date, Referral_Source, 
...
ready_to_review2 <- ready_to_review %>% filter(Participant_Type=="MDD") 

if(nrow(ready_to_review2)==0){

  cat("  \n",  "  \n###",  "No MDDs ready to reivew today.",  "  \n###", "Number of HVs ready to review:", nrow(ready_to_review))

} else {
  
  cat("  \n###", "Number of MDDs to review today:", nrow(ready_to_review2))
  
  for(i in seq(nrow(ready_to_review2))) {
  # iter=3
    iter <- as.numeric(i)
  
    temp <- ready_to_review2 %>% slice(eval(iter))
...
screening2 <- screening %>% filter(Participant_Type=="MDD") %>% filter(!is.na(FIRST_NAME) & !is.na(LAST_NAME)) %>% filter(FIRST_NAME!="UNKNOWN" & LAST_NAME!="UNKNOWN")

if(nrow(screening2)==0) {

  cat("  \n",  "  \n###",  "No MDDs currently screening.",  "  \n###", "Number of HVs screening:", nrow(screening))

} else {
  
  cat("  \n###", "Number of MDDs screening:", nrow(screening2))

  for(i in seq(nrow(screening2))) {
    # iter=3
    iter <- as.numeric(i)
  
    temp <- screening2 %>% slice(eval(iter))
