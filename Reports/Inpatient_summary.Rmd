...
daily_mfqs <- s_mfq1d_final %>% filter(Initials %in% inpatient_list) %>% 
  select(Initials, s_mfq1d_date, s_mfq1d_date3, s_mfq1d_tot) %>% arrange(s_mfq1d_date)
...
for(i in seq(nrow(current_inpatients))) {
  ...
  temp_daily_mfq <- daily_mfqs %>% filter(Initials==Participant) %>% arrange(as.numeric(s_mfq1d_date))
...
rm(temp_demo, temp_meds, temp_psych, temp_daily_mfq, questionnaire_data, med_data, daily_mfq_data, FitFlextableToPage2)
