...
crisisChildFilename=paste0(database_location, "/COVID19/Summary Numbers/2021-01-28_just_crisis/crisisAndCovid_numbers_basedOn_CrisisTminus1_precovidPlusIds_2021-01-28.csv")
 if(file.access(crisisChildFilename, mode=0) != 0 ){
      warning(paste("can't access ",crisisChildFilename))
    }
covidAndCrisis=read.csv(crisisChildFilename)
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
covidAndCrisis=read.csv(paste0(database_location, "/COVID19/Summary Numbers/2021-01-28_just_crisis/parent_crisisAndCovid_numbers_basedOn_CrisisTminus1_precovidPlusIds_2021-01-28.csv"))
summaryOfCovidAndCrisis=cbind(as.character(covidAndCrisis[,1]), covidAndCrisis[,2],covidAndCrisis[,4]+ covidAndCrisis[,7],covidAndCrisis[,5]+ covidAndCrisis[,8])
#colnames(summaryOfCovidAndCrisis)=c("Dates", "Completed CRISIS", "and have PreCovid Data", "and Completed MFQ+Scared")
summaryOfCovidAndCrisis %>% kable(., col.names = c("Dates", "Total CRISIS completed","and MFQ/Scared before 03/17/2019", "and completed MFQ and SCARED")) %>%
   kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
