...
y123fu=add_column(y1fu,y2fu) %>% add_column(., y3fu)
y123fu= y123fu %>% replace_na(list(y1MDD = 0, y2MDD = 0,y3MDD = 0,y1HV = 0, y2HV = 0,y3HV = 0))
#totalMDDcol=y123fu%>% select(y1MDD,y2MDD,y3MDD) %>% colSums()
totalMDDrow=y123fu%>% select(y1MDD,y2MDD,y3MDD) %>% rowSums()
#totalHVcol=y123fu%>% select(y1HV,y2HV,y3HV) %>% colSums()
totalHVrow=y123fu%>% select(y1HV,y2HV,y3HV) %>% rowSums()
...
y123fuDisplay<-y123fu %>% select("Descrip","Date","y1MDD","y2MDD","y3MDD","y1HV","y2HV","y3HV","totalMDDrow","totalHVrow","totalall")


```
Summamry of Follow ups:
```{r echo=FALSE, warning=FALSE, message=FALSE}

y123fuDisplay %>% kable(., col.names = c("Descrip","Date","12m MDD","24m MDD","36m MDD","12m HV","24m HV","36m HV","total MDD ","total HV","total")) %>%
  kable_styling(position = "left", bootstrap_options = c("striped", "hover", "condensed"))
